...
setInterval(() => {
  http.get(`http://hussen55.glitch.me/`); /// حط اسم المشروع تبعك name تعديل مهم بدل
}, 280000);
  ...
  if (message.content.startsWith(prefix + "bc")) {
    ...
    if (message.guild.interval)
      return message.reply(
        "**بث آخر قيد التشغيل ، الرجاء الانتظار حتى ينتهي**"
      );
              ...
              m.edit("**>>> ادخل اسم الرتبة من فضلك**").then(ms => {
                message.channel
                  ...
                  .then(async collected => {
                    ...
                    if (members.length == 0)
                      return message.reply("**لم يتم العثور على الرقم.**");
                    else {
                      const msg = await message.channel.send(
                        `**جاري إرسال الرسالة إلى ${members.length} عضواً...**`
                      );
                      var count = 0;
                      var ycount = 0;
                      var xcount = 0;
                        ...
                        if (!members[count]) {
                          ...
                          msg.edit(
                            new Discord.RichEmbed()
                              .setDescription(
                                `** :mailbox_with_mail:  ؛ تم أرسال الرسالة الى  ${ycount} عضواً\n:lock: ؛ و لم أستطع أرسال الرسالة إلى ${xcount} عضواً**`
                              )
                              .setTimestamp()
                          );
    ...
    let args = message.content
      ...
      .slice(1)
      .join(" ");
    if (!message.author.id === "427011300082581504") return; ///تعديل مهم حط الايدي تبعك
...
client.on("ready", () => {
  client.user.setActivity(`HusśseinHi`, {
    ///تعديل مهم حالة البوت
    type: "WATCHING"
  }); ///تعديل حالة البوت
});
